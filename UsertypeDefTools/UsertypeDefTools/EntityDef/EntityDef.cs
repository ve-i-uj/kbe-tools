...
public class EntityDef
{
    ...
    public class Method
    {
        ...
        public Method(XmlNode node)
        {
            ...
            foreach (XmlNode item in argNodes)
            {
                ...
                IType type = BaseType.AllTypes.Find(e => e.Type == typeStr);
                if (type == null)
                {
                    type = item.GetIType();
                    if (type == null)
    ...
    void Reset(string content)
    {
        ...
        //Implements
        var implements = root.SelectSingleNode("Interfaces");
        if (implements != null)
        {
    ...
    public static void LoadFromFile(string dir)
    {
        if (!Directory.Exists(dir))
            throw new ArgumentException(string.Format("'{0}' is not exists.", dir));
    ...
    void Save(string dir)
    {
        ...
        if (Volatile != null && (Volatile.Position.HasValue || Volatile.Pitch.HasValue || Volatile.Roll.HasValue || Volatile.Yaw.HasValue))
        {
